...
SELECT * FROM tbl_book ;
	...
	CREATE TABLE tbl_book_copies (
        ...
        CONSTRAINT fk_book_id2 FOREIGN KEY(book_loans_BookID) REFERENCES tbl_book(book_BookID) ON UPDATE CASCADE ON DELETE CASCADE,
		CONSTRAINT fk_branch_id2 FOREIGN KEY(book_loans_BranchID) REFERENCES tbl_library_branch(library_branch_BranchID) ON UPDATE CASCADE ON DELETE CASCADE
    );
...
BEGIN
	SET bookTitle = COALESCE(bookTitle, 'The Lost Tribe');
    SET branchName = COALESCE(branchName, 'Sharpstown');
    
    
    SELECT copies.book_copies_BranchID AS 'BranchID' , branch.library_branch_BranchName AS 'Branch Name' , copies.book_copies_No_of_Copies AS 'Number of Copies', book.book_Title AS 'Book Title'
...
CREATE PROCEDURE bookCopiesAtallbranch(
...
BEGIN
    ...
    SELECT copies.book_copies_BranchID AS 'BranchID' , branch.library_branch_BranchName AS 'BranchNAame', copies.book_copies_No_of_Copies AS 'Number of Copies', book.book_Title AS 'Book Title'
...
DELIMITER //
CREATE PROCEDURE branchinfo(
	IN branchName VARCHAR(70),
    IN Duedate VARCHAR(50)
    )

BEGIN 
	SET branchName = COALESCE(branchName, 'Sharpstown');
    SET Duedate = COALESCE(DueDate , NULL);
	...
	SELECT Branch.library_branch_BranchName AS 'Branch Name',  Book.book_Title 'Book Name',
	   ...
	   FROM tbl_book_loans AS Loans
		...
		WHERE Loans.book_loans_DueDate = Duedate AND Branch.library_branch_BranchName = BranchName;
END;
...
SHOW CREATE PROCEDURE bookCopiesAtAllSharpstown;
SHOW CREATE PROCEDURE branchinfo;
